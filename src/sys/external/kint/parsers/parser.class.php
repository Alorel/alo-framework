<?php
   ...
   abstract class kintParser extends kintVariableData {
      ...
      private static function _parse_array(&$variable, kintVariableData $variableData) {
         ...
         if ($tabular) {
            ...
            foreach ($variable as $key => & $val) {
               if ($key === self::$_marker || (Kint::$keyFilterCallback && call_user_func(Kint::$keyFilterCallback, $key, $val) === false)
               ) {
                  continue;
               }
      ...
      private static function _parse_object(&$variable, kintVariableData $variableData) {
         ...
         foreach ($array as $key => & $value) {
            if (Kint::$keyFilterCallback && call_user_func_array(Kint::$keyFilterCallback, [$key, $value]) === false
            ) {
               continue;
            }
   ...
   class kintVariableData {
      ...
      protected static function _escape($value, $encoding = null) {
         ...
         if (function_exists('mb_encode_numericentity')) {
            return mb_encode_numericentity(
               $value, [0x80, 0xffff, 0, 0xffff,], $encoding
            );
         } else {
            return $value;
         }
      ...
      protected static function _detectEncoding($value) {
         ...
         $md5 = md5($value);
         foreach (array_merge(['UTF-8'], Kint::$charEncodings) as $encoding) {
      ...
      protected static function _isSequential(array $array) {
         return Kint::$hideSequentialKeys ? array_keys($array) === range(0, count($array) - 1) : false;
      }
      ...
      protected static function _substr($string, $end, $encoding = null) {
         ...
         return function_exists('mb_substr') ? mb_substr($string, 0, $end, $encoding) : substr($string, 0, $end);
      }
