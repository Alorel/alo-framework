<?php
   ...
   /**
    * Kint is a zero-setup debugging tool to output information about variables and stack traces prettily and
    * comfortably. https://github.com/raveren/kint
    */
   ...
   class Kint {
      ...
      protected static $_statements = ['include', 'include_once', 'require', 'require_once'];
      ...
      public static function dump($data = null) {
         ...
         if ($names === [null] && func_num_args() === 1 && $data === 1) {
            $call = reset($trace);
         ...
         $data = func_num_args() === 0 ? ["[[no arguments passed]]"] : func_get_args();
      ...
      /**
       * getter/setter for the enabled parameter, called at the beginning of every function as getter, also
      ...
      private static function _getPassedNames($trace) {
         ...
         $parameters = [];
         $blacklist = ['null', 'true', 'false', 'array(...)', 'array()', '"..."', 'b"..."',];
         ...
         return [$parameters, $modifier, $callee, $previousCaller];
      }
      ...
      private static function _removeAllButCode($source) {
         ...
         $commentTokens = [T_COMMENT => true, T_INLINE_HTML => true, T_DOC_COMMENT => true];
         ...
         foreach ($trace as $step) {
            ...
            if (in_array($step['function'], self::$_statements)) {
               if (empty($step['args'])) {
                  ...
                  $args = [self::shortenPath($step['args'][0])];
               }
      ...
      public static function shortenPath($file, $line = null) {
         ...
         $url = str_replace(['%f', '%l'], [$file, $line], self::$fileLinkFormat);
         $class = (strpos($url, 'http://') === 0) ? 'class="kint-ide-link"' : '';

         return "<span style='text-decoration: underline'><a {$class} href=\"{$url}\">{$shortenedName}</a></span> line <i>{$line}</i>";
      }
