<?php
   ...
   class Kint_Decorators_Rich extends Kint {
      ...
      protected static function _wrapEnd($callee, $prevCaller) {
         ...
         $calleeInfo = isset($callee['file']) ? 'Called from ' . self::shortenPath($callee['file'], $callee['line']) : '';

         return $calleeInfo || $callingFunction ? "<footer>{$calleeInfo}{$callingFunction}</footer></div>" : "</div>";
      }
